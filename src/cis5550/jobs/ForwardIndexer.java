...
public class ForwardIndexer {
	private static final Logger logger = Logger.getLogger(ForwardIndexer.class);
	...
	public static void run(FlameContext ctx, String[] urlArg) throws Exception {
		try {
			...
			logger.info("forward indexer: lexicon size: " + lexicon.size());
			logger.info("forward indexer: stopwords size: " + stopwords.size());
			logger.info("forward indexer: num workers: " + ctx.getKVS().numWorkers());
			...
			FlamePairRDD fwdIndexExec = pairRDD.flatMapToPair(pair -> {
				...
				for (String word : words) {
					...
					// if (stemWord.compareTo(word) != 0 && (!stopwords.contains(stemWord) && lexicon.contains(stemWord))) {
				...
				for (String word : wordSet) {
					String stemWord = getStemWord(word);
					if (!wordCnt.containsKey(stemWord) && (!stopwords.contains(stemWord) && lexicon.contains(stemWord))) {
						wordCnt.put(stemWord, wordCnt.get(word));
						wordPos.put(stemWord, wordPos.get(word));
					}
				}
			...
			System.out.println("forward indexer: fwdIndex size: " + ctx.getKVS().count("forwardIndex"));
			logger.info("forward indexer: forwardIndex size: " + ctx.getKVS().count("forwardIndex"));
